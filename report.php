...
if(!isset($_SESSION['logged_in'])) 
{
    ...
    $data .='</table>';
    $data .= '<h5 class="subTitle">All Department: </h5>
        <table border="1" cellspacing="0" cellpadding="5">
            <tr>
                <th>Department ID</th>
                <th>Name</th>
    ...
    $result = mysqli_query($con, $select);
    $totalDept = mysqli_num_rows($result);
    while ($row = mysqli_fetch_assoc($result)) {
        $arr = array($row["deptName"]=> $row["d_id"]);
        $catIDArray = array_merge($catIDArray, $arr);

        $data .= '<tr><td>'.$row["d_id"].'</td><td>'.$row["name"].'</td><td>'.$row["hod_name"].'</td><td>'.$row["description"].'</td><td>'.$row["totalsection"].'</td></tr>';
    }
    ...
    <h5 class="subText"> Total Department: '.$totalDept.'</h5>
